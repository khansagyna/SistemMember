...
export function useAddTransaction(onSuccess?: () => void) {
  ...
  const filteredMembers = allMembers.filter(
    (m) =>
      m.name.toLowerCase().includes(searchMember.toLowerCase()) || m.phone.includes(searchMember)
  );
  ...
  useEffect(() => {
    ...
    if (next >= activePromo.target_transaction && Number(amount) >= activePromo.minimum_amount) {
